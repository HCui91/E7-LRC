...
freq, volt, delta_volt, y0, y1, delta_y = np.loadtxt("121020171018.csv",delimiter="	",unpack=True) 
freq_1, volt_1, delta_volt_1, y0_1, y1_1 = np.loadtxt("121020171122.csv",delimiter="	",unpack=True)
...
#Phase angle error
delta_y1y0=np.sqrt((delta_volt/y1)**2+(delta_volt/y0)**2)*(y1/y0)
delta_phase = delta_y1y0/np.sqrt(1-(y1/y0)**2)
delta_y1y0_1=np.sqrt((delta_volt_1/y1_1)**2+(delta_volt_1/y0_1)**2)*(y1_1/y0_1)
delta_phase_1 = delta_y1y0_1/np.sqrt(1-(y1_1/y0_1)**2)

#correcting phase angle
for x in range(0,36):
...
#Amplitude graph
plt.errorbar(freq,volt,yerr=delta_volt,fmt='o',label="Low damping")
...
plt.legend(loc="best")
plt.xlim(6e4-1000,26e4+1000)
plt.ylim(0.5,4)
plt.grid()
#plt.show()
#Save graph
plt.savefig("E7 amplitude")

#Phase angle graph
plt.figure(2,figsize=(10,7))
plt.errorbar(freq,phase,yerr=delta_phase,fmt='o',label="Low damping")
plt.errorbar(freq_1,phase_1,yerr=delta_phase_1,fmt='D',label="High damping")
#plt.plot((omega,omega), (0, 4), 'k--',label="Theoretical amplitude resonance")
plt.plot((50000,280000),(np.pi/2,np.pi/2),'--',label="Phase angle=pi/2")
